...
setwd("C:/Users/nikaf/OneDrive/Desktop/LS AMF/code")
...
#Figure 2: AMF in canopy gaps versus intact understory (controlling for seedling biomass, and not)
...
m1.s<-glm(X.col~light.per*nitro+tot.mass+type,data=data.s, family=binomial(), weights=views)
summary(m1.s)

#Figure 3: AMF versus light and nitrogen treatments
...
m2.z<-glm(tot.mass~light.per+nitro+X.col+tot.nod.mass+type,data=data.z, family= gaussian(link="log"))
summary(m2.z)

m2.s<-glm(tot.mass~light.per+nitro+X.col+tot.nod.mass+type,data=data.s, family= gaussian(link="log"))
summary(m2.s)
...
#by species
cor.test(data.p$tot.nod.mass, data.p$tot.mass, method = "spearman")
cor.test(data.z$tot.nod.mass, data.z$tot.mass, method = "spearman")
cor.test(data.s$tot.nod.mass, data.s$tot.mass, method = "spearman")

#Figure 1b: Seedlng biomass versus SNF
...
#by species
cor.test(data.p$X.col, data.p$tot.mass, method = "spearman")
cor.test(data.z$X.col, data.z$tot.mass, method = "spearman")
cor.test(data.s$X.col, data.s$tot.mass, method = "spearman")

#Figure 1c: Seedling biomass versus AMF
...
# Supplemental Figure 4 (Fig. 2 by species)
...
Fig.4.species1<-ggarrange(p.AMF.gap.understory,z.AMF.gap.understory,s.AMF.gap.understory,
                          labels=c("a","c","e"),
                          ncol=1,nrow=3,
                          hjust=c(-3,-3,-3),
                          vjust=c(1.5,1.5,1.5),
...
Fig.4.species2<-ggarrange(p.AMF.per.g.gap.understory,z.AMF.per.g.gap.understory,s.AMF.per.g.gap.understory,
                          labels=c("b","d","f"),
                          ncol=1,nrow=3,
                          hjust=c(-3,-3,-8),
                          vjust=c(1.5,1.5,1.5),
...
ggarrange(Fig.4.species1,NULL,Fig.4.species2,
          ncol=3,nrow=1,
          widths=c(1,0.15,1))

#Supplemental Figure 5 (Fig. 3 by species)
fig5.p<-ggplot(data.p, aes(x=light.per, y=X.col, weight=views))+
...
fig5.z<-ggplot(data.z, aes(x=light.per, y=X.col, weight=views))+
  geom_point(aes(shape=nitro),size=2.5,alpha=0.7)+
  scale_shape_manual(values=c(1, 16))+
  xlab("")+
  theme_pubr()+
...
fig5.s<-ggplot(data.s, aes(x=light.per, y=X.col, weight=views))+
  geom_point(aes(shape=nitro),size=2.5,alpha=0.7)+
  scale_shape_manual(values=c(1, 16))+
  xlab("")+
  theme_pubr()+
...
ggarrange(fig5.p,fig5.z,fig5.s,NULL,
          ncol=1,nrow=4,
          labels=c("a","b","c",""),hjust=-2,vjust=.7,
          heights=c(1,1,1,.2),
          font.label = list(size=20))

#Supplemental Figure 1 (fig. 1 for P. mac)
...
#Supplemental Figure 2 (Fig. 1 for Z. longifolia)
...
#Supplemental Figure 3 (Fig. 1 for S. microstachyum)
...
m3<-glm(tot.mass~S.num+nitro+light.per+X.col+tot.Nfixd+type,data=data, family= gaussian(link="log"))
summary(m3)

#subsetting data by species again to include Nfixd variable in subsets
...
#by species
m3.p<-glm(tot.mass~light.per+nitro+X.col+tot.Nfixd+type,data=data.p, family= gaussian(link="log"))
summary(m3.p)
